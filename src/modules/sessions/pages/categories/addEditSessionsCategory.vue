...
export default {
  setup() {
    ...
    const functionCategory = () => {
      if (route.name == "sessionsAddCategory") {
        add.value = true;
        edit.value = false;
      } else if (route.name == "sessionsEditCategory") {
        edit.value = true;
        add.value = false;
    ...
    const addData = () => {
      ...
      if (form.value.orderNumber && form.value.name) {
        adminServices
          .addSessionCategory(orgId, appID, form.value)
          .then((resp) => {
    ...
    const onGetCategoryDetailes = () => {
      $q.loading.show();
      adminServices
        .detailesSessionCategory(orgId, appID, route.params.id)
        .then((res) => {
    ...
    const editData = () => {
      ...
      if (form.value.orderNumber && form.value.name) {
        adminServices
          .editSessionCategory(orgId, appID, route.params.id, form.value)
          .then((resp) => {
            if (resp.data.statusCode == 200) {
              $q.notify({
                color: "green",
