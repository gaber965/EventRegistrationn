<template>
    <q-page padding>
      ...
      <div class="q-pa-md table row exbhibitors-category" id="List">
          ...
          <div class="row exbhibitors-category__contet">
            <div class="col-md-2 col-12 exbhibitors-category__filter">
              <div class="add-category-btn">
                <router-link class="menu-link" :to="{ name: 'addInterests' }">
            ...
            <template v-slot:body-cell-actions="props">
              <q-td :props="props" class="actions">
                <q-btn
                  ...
                  @click.stop="editInterests(props.row.id)"
                  class="edit icon"
                ></q-btn>
        ...
        <deleteDialoge @fireEvent="onDelete" v-model="dialog" :name="'Interests'" />
      </div>
    </q-page>
  </template>
  <script>
  ...
  export default {
    ...
    setup() {
      ...
      const onGetAllInterests = () => {
        $q.loading.show();
        services
          .getAllInterests(orgId, appId)
          .then((res) => {
            if (res.data.statusCode == 200) {
              allInterests.value = res.data.result.items;
              $q.loading.hide();
            } else {
              $q.loading.hide();
              $q.notify({
      ...
      const openDialog = (id) => {
        interestsId.value = id;
        dialog.value = true;
      };
      const onDelete = () => {
        $q.loading.show();
        dialog.value = false;
        services.deleteData(orgId, appId, interestsId.value).then((res) => {
          if (res.data.statusCode == 200) {
            onGetAllInterests();
            $q.notify({
              color: "green",
      ...
      return {
        ...
        pagesNumber: computed(() =>
          Math.ceil(allInterests.value.length / pagination.value.rowsPerPage)
        ),
        model,
        options,
        update,
